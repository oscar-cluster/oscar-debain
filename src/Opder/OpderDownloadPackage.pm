...
sub NEW
{
    ...
    my $OpderDownloadPackageLayout = Qt::GridLayout(this, 1, 1, 1, 6, '$OpderDownloadPackageLayout');

    my $layout20 = Qt::VBoxLayout(undef, 11, 6, '$layout20');
    ...
    $layout20->addWidget(downloadLabel);
    ...
    $layout20->addWidget(packageLabel);

    packageProgressBar = Qt::ProgressBar(this, "packageProgressBar");
    packageProgressBar->setSizePolicy(Qt::SizePolicy(7, 0, 0, 0, packageProgressBar->sizePolicy()->hasHeightForWidth()));
    packageProgressBar->setProgress(int(0));
    packageProgressBar->setPercentageVisible(0);
    $layout20->addWidget(packageProgressBar);
    my $spacer = Qt::SpacerItem(20, 10, &Qt::SizePolicy::Minimum, &Qt::SizePolicy::Expanding);
    $layout20->addItem($spacer);

    packageLabel_2 = Qt::Label(this, "packageLabel_2");
    packageLabel_2->setPaletteForegroundColor(Qt::Color(0, 0, 0));
    packageLabel_2_font = Qt::Font(packageLabel_2->font);
    packageLabel_2_font->setBold(1);
    packageLabel_2->setFont(packageLabel_2_font);
    packageLabel_2->setText(trUtf8(" Total Progress:"));
    packageLabel_2->setAlignment(int(&Qt::Label::AlignVCenter | &Qt::Label::AlignLeft));
    $layout20->addWidget(packageLabel_2);

    totalProgressBar = Qt::ProgressBar(this, "totalProgressBar");
    totalProgressBar->setSizePolicy(Qt::SizePolicy(7, 0, 0, 0, totalProgressBar->sizePolicy()->hasHeightForWidth()));
    totalProgressBar->setProgress(int(0));
    totalProgressBar->setPercentageVisible(0);
    $layout20->addWidget(totalProgressBar);
    ...
    $OpderDownloadPackageLayout->addLayout($layout20, 0, 0);
  ...
  if ((-e $opdcmd) && (-x $opdcmd))
    {
      ...
      if (!($dlProc->isRunning()))
        { 
          ...
          totalProgressBar->setTotalSteps(0);
          totalProgressBar->setPercentageVisible(1);
          totalProgressBar->setCenterIndicator(1);
        }
...
#########################################################################
  ...
  totalProgressBar->setTotalSteps(scalar(@dlPackages));
...
#########################################################################
  ...
  totalProgressBar->setProgress($dlPhase-1);
